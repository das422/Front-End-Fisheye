...
export const mediaFilter = mediasTemplate => {
  ...
  filter.addEventListener("change", () => {
    ...
    if (selected === "popularite") {
      const popularMedia = mediasTemplate.medias.sort((a, b) => b.likes - a.likes);
      mediasTemplate.medias = popularMedia;
    } else if (selected === "date") {
      const dateMedia = mediasTemplate.medias.sort((a, b) => new Date(b.date) - new Date(a.date));
      mediasTemplate.medias = dateMedia;
    } else if (selected === "titre") {
      const titleMedia = mediasTemplate.medias.sort((a, b) => a.title.localeCompare(b.title));
